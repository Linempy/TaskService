...
**`POST /api/v1/tasks/{id}/process`** нужен для **повторного запуска** после терминальных статусов **`FAILED`** или **`CANCELLED`**: переводит задачу обратно в `PENDING`, сбрасывает флаг отмены и результат и снова ставит в очередь.

Таким образом, отдельный эндпоинт «запуска обработки» можно понимать как **явный retry**
